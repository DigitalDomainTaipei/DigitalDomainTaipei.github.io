...
TRACKER.on('track', function(faces) {
    ...
    faces.data.forEach(function(rect) {
        ...
        var shape_geometry = predictShape( document.getElementById(face_id+'_256') );
        update_shape_geometry(shape_geometry);
    });
});
...
function cropFace(rect, face_id) {
    ...
    var canvas_256 = document.createElement('canvas');
    canvas_256.id = face_id + '_256';
    canvas_256.className = "canvas-face";
    canvas_256.width = 256;
    canvas_256.height = 256;
    canvas_256.getContext('2d').drawImage(img, x*ratio, y*ratio, w * ratio, h * ratio, 0, 0, 256, 256);
    document.body.appendChild(canvas_256);
